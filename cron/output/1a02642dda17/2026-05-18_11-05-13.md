...
## 📊 youtubd 每日处理报告 — 2026-05-18

### ✅ 已完成步骤

**1. 随机抽取 5 个视频 → todolist.txt**
从 1259 个未处理视频中按频道权重加权随机抽取 5 个：

| # | 视频 | 频道 | 时长 | 状态 |
|---|------|------|------|------|
| 1 | `VsfAKoXiV4Y` x86时代终结？高通骁龙X Elite | 老石谈芯 | 14min | ⛔ token保护跳过 |
| 2 | `FBZvh4cMySc` CIA CAN HEAR YOUR CONVERSATIONS | (未知) | 1min | ❌ 网络不可达 |
| 3 | `NbldZVdusKo` Hermes Agent和OpenClaw有什么区别 | 最佳拍档 | 18min | ⛔ token保护跳过 |
| 4 | `sAW-323CUOU` 别卷CUDA了！DeepSeek开源TileKernels | 为什么叫QQ | 9min | ❌ 网络不可达 |
| 5 | `dGVfFPDe198` 在深圳参加芯片大会 | 老石谈芯 | 9min | ❌ 网络不可达 |

### ❌ 阻塞问题

**NotebookLM 不可用** — CSRF session 已过期，且无法在 headless 环境下重新登录（需要 X server）。已尝试更新 `certifi` 但未能解决。

**YouTube 网络不可达** — 服务器上的 Clash Meta VPN 代理路由异常，`youtube.com` / `google.com` 通过 `198.18.0.0/30` TUN 接口时 SSL 握手失败（`SSL_ERROR_SYSCALL`）。GitHub 等其他 HTTPS 站点正常。

### 📋 待处理队列

5 个视频仍保留在 `todolist.txt` 中，**未标记为已处理**，下次 cron 运行时会重新尝试。
...
| 已处理 | 141 |
| 未处理 | **1259** |
| 当前 todolist | 5 个视频（等待处理） |

### 建议

1. 检查 Clash Verge / Meta 代理配置，确保 YouTube/Google 路由正常
2. 或使用 `notebooklm login` 在桌面环境中重新认证 NotebookLM
3. 下次 cron 运行会重试这些视频
