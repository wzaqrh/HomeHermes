...
2. `OBSIDIAN_VAULT_PATH` 环境变量（指向 brain-vault，默认 `~/MyDoc/brain-vault`）
...
### NotebookLM 已恢复
NotebookLM CLI 当前可用。如需使用 NotebookLM，优先调用 `notebooklm-to-brainvault` 技能。本技能作为字幕抓取的 fallback 路径使用。
...
Agent 必须检查返回值。失败时不创建任何文件，不标记 processed。
