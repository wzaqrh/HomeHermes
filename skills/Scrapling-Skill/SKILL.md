...
💡 Quick Static Page Capture: If you only need to scrape static web pages without JavaScript rendering or anti-bot bypass, you can directly use the `scrapling extract get` command without running `scrapling install --force`, avoiding sudo permission issues.
